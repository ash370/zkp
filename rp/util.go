...
func Inner_produ(a []*big.Int, b []*big.Int) *big.Int {
	...
	for key, _ := range a {
		...
		tmp.Mod(tmp, P)
		sum.Add(sum, tmp)
		//sum.Mod(sum, P_1.Sub(P, big.NewInt(1)))
		sum.Mod(sum, P)
	}
	return sum
}
...
func Generate_s(n int64) []*big.Int {
	...
	for i := n; i > 0; i-- {
		...
		s = append(s, r)
		/*all is 1 (from github/RangProof)*/
...
func GenerateY(y big.Int, n int64) []*big.Int {
	...
	for ; i < n; i++ {
		...
		//tmp.Mod(&tmp, P_1.Sub(P, big.NewInt(1)))
		tmp.Mod(&tmp, P)
...
func GenerateZ1(z big.Int, n int64) []*big.Int {
	...
	z1 = big.NewInt(1)
	z1.Neg(&z)
	//z1.Mod(z1, P_1.Sub(P, big.NewInt(1)))
	z1.Mod(z1, P)
	for i := n; i > 0; i-- {
...
func GenerateH1(H []bn254.G1Affine, y big.Int, n int64, order *big.Int) []bn254.G1Affine {
	yn := GenerateY(y, n)
